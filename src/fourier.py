...
def valid_fourier_K(p: int, requested: list[int] | tuple[int, ...]) -> list[int]:
    max_k = (p - 1) // 2
    d = 2 * p
    return [int(k) for k in requested if int(k) >= 1 and int(k) <= max_k and 4 * int(k) < d]


def make_fourier_basis(p: int, K: int, dtype: torch.dtype = torch.float64) -> tuple[torch.Tensor, torch.Tensor]:
    """Return Q and P for the real Fourier subspace S*_K in R^{2p}."""
    if K < 1 or K > (p - 1) // 2:
        raise ValueError(f"K={K} invalid for p={p}")
    d = 2 * p
    t = torch.arange(p, dtype=dtype)
    cols = []
    for k in range(1, K + 1):
        angle = 2.0 * math.pi * k * t / p
        cos = torch.cos(angle)
        sin = torch.sin(angle)
        for block, vec in ((0, cos), (0, sin), (1, cos), (1, sin)):
            col = torch.zeros(d, dtype=dtype)
            start = block * p
            col[start : start + p] = vec
            cols.append(col)
...
def validate_fourier(p: int, K: int, trials: int = 20) -> list[str]:
    messages: list[str] = []
    Q, P = make_fourier_basis(p, K)
    I = torch.eye(Q.shape[1], dtype=Q.dtype)
    assert torch.allclose(Q.T @ Q, I, atol=1e-8), "Fourier Q is not orthonormal"
    messages.append(f"Q.T @ Q close to identity for K={K}")
